...
if args.eval_tail_size:
    ...
    elif type_of_file == 'val':
        file_list = {'Tail': 'ood_val_tail.json', 'Head': 'ood_val_head.json', 'All': 'ood_val_all.json'}
    result = {}
    for setup, ques_file_path in file_list.items():
    # for setup, ques_file_path in file_list.items():
        gqa_eval = GQAEval(result_eval_file, '/home/majie/code/alo/data/gqa/gqaood/annotations/questions/' +
                           ques_file_path, choices_path=None, EVAL_CONSISTENCY=False)
    ...
    print('\nRESULTS:\n')
    msg = 'Accuracy (all, tail, head,): %.2f, %.2f, %.2f' % (result['All'], result['Tail'], result['Head'] )
    print(msg)