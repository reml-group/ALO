...
class VQARUBiMetrics(VQAAccuracies):
    ...
    def reset_oe(self):
        ...
        for key in ['', '_q', '_all']:
            ...
            if self.dataset.split == 'test':
                ...
                # self.path_logits = os.path.join(self.dir_rslt[key], 'logits.pth')
                ...
                # self.idx = 0
                #
                # path_aid_to_ans = os.path.join(self.dir_rslt[key], 'aid_to_ans.json')
            ...
            if 'test' not in self.dataset.split:
                call_to_prog = 'python -m block.models.metrics.compute_oe_accuracy ' \
                               + '--dir_vqa {} --dir_exp {} --dir_rslt {} --epoch {} --split {} --logs_name {} --rm {} &' \
                                   .format(self.dir_vqa, self.dir_exp, self.dir_rslt[key], self.engine.epoch,
                                           self.dataset.split, logs_name, self.rm_dir_rslt)
                Logger()('`' + call_to_prog + '`')
    ...
    def compute_tdiuc_metrics(self):
        ...
        for key in ['', '_all', '_q']:
            ...
            for tp in types:
                acc = 100 * (len(self.res_by_type[key][tp + '_t']) / len(
                    self.res_by_type[key][tp + '_t'] + self.res_by_type[key][tp + '_f']))
                sum_acc.append(acc + eps)
                Logger()(f"Accuracy {key} for class '{tp}' is {acc:.2f}")
                Logger().log_value('{}_epoch.tdiuc{}.perQuestionType.{}'.format(self.mode, key, tp), acc,
                                   should_print=False)
            ...
            for tp in types:
                ...
                Logger().log_value('{}_epoch.tdiuc{}.perQuestionType_norm.{}'.format(self.mode, key, tp), acc,
                                   should_print=False)
